...
def split_text(text: str, n=100, character=' ') -> List[str]:
    ...
    return [
        character.join(text[i:i + n]).strip() for i in range(0, len(text), n)
    ]
...
def generate_csv():
    ...
    for idx, book in enumerate(iter_books()):
        ...
        for sentence in book.sentences:
            ...
            for sub_sentence in sentence.split('\n'):
                ...
                # Make sure there's more than 1 word and less than x words
                if len(sub_sentence.split()) < 2:
                    continue
    ...
    pandas.DataFrame(to_write).to_csv(
        '/opt/mauve/rag/something_noindex.csv',
        index=False
    )
...
def main(
    ...
    assert os.path.isfile('/opt/mauve/rag/something_noindex.csv'), 'Please provide a valid path to a csv file'

    # You can load a Dataset object this way
    dataset = load_dataset(
        'csv',
        data_files=['/opt/mauve/rag/something_noindex.csv'],
        split='train',
        delimiter=',',
