...
from mauve.bin.push_to_splunk import process_files
...
class TestCompressFiles(TestCase):
    ...
    def test_compress_files(self, push_to_splunk_mock):
        book = Book(title='t', author='a', year_published=0)
        book.set_content_location(self.text_path_1)
        book.tokens  # Load the tokens file

        book = Book(title='t', author='a', year_published=0)
        book.set_content_location(self.text_path_2)
        book.tokens  # Load the tokens file

        process_files(num_processes=1)
        self.assertEqual(push_to_splunk_mock.call_count, 2)
